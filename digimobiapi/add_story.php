...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ✅ Yetki kontrolü
    if ($is_moderator) {
        // Moderator: Her zaman hikaye paylaşabilir
        ...
        $can_share_story = ($permissions['hikaye_paylasabilir'] ?? false) && !$is_before_event && !$is_after_access;
    } else {
    ...
    try {
        ...
        $media_type = strpos($file['type'], 'video') !== false ? 'video' : 'foto';
        
        // Insert story into database
        $stmt = $pdo->prepare("
            INSERT INTO medyalar (kullanici_id, dugun_id, dosya_yolu, tur, aciklama, created_at)
            VALUES (?, ?, ?, 'hikaye', ?, NOW())
        ");
        $stmt->execute([$user_id, $event_id, 'uploads/stories/' . $filename, $description]);
