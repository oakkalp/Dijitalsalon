...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Delete story files (original, thumbnail, preview)
        $file_path = $story['dosya_yolu'] ?? null;
        $thumbnail_path = $story['kucuk_resim_yolu'] ?? null;
        $preview_path = $story['onizleme_yolu'] ?? null;
        ...
        if ($file_path) {
            ...
            if (file_exists($full_path)) {
                @unlink($full_path);
                error_log("Deleted original story file: $full_path");
            }
        }
        ...
        if ($thumbnail_path) {
            ...
            if (file_exists($full_thumbnail_path)) {
                ...
                error_log("Deleted thumbnail story file: $full_thumbnail_path");
            }
        }
        ...
        if ($preview_path) {
            ...
            if (file_exists($full_preview_path)) {
                ...
                error_log("Deleted preview story file: $full_preview_path");
            }
        }
