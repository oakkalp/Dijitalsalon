...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        ...
        foreach ($comments as $comment) {
            $formatted_comments[] = [
                ...
                'user_avatar' => $comment['user_avatar'] ? 'https://dijitalsalon.cagapps.app/' . $comment['user_avatar'] : null,
    ...
    try {
        ...
        if ($is_moderator) {
            ...
            // ✅ Yetkiler array olarak geliyorsa (["yorum_yapabilir", ...]) in_array kullan
            // ✅ Ya da object olarak geliyorsa ({"yorum_yapabilir": true, ...}) key kontrolü yap
            if (is_array($permissions) && isset($permissions[0])) {
                $has_comment_permission = in_array('yorum_yapabilir', $permissions);
            } else {
                $has_comment_permission = ($permissions['yorum_yapabilir'] ?? false);
            }
            
            $can_comment = $has_comment_permission && !$is_before_event && !$is_after_access;
        } else {
        ...
        $stmt = $pdo->prepare("
            SELECT d.id as event_id, d.baslik as event_title
            FROM dugunler d
            WHERE d.id = ?
        ");
        $stmt->execute([$media['dugun_id']]);
        ...
        $log_details = json_encode([
            ...
            'comment_text' => substr($content, 0, 200), // İlk 200 karakter
        ...
        echo json_encode([
            ...
            'comment' => [
                ...
                'user_avatar' => $user['profil_fotografi'] ? 'https://dijitalsalon.cagapps.app/' . $user['profil_fotografi'] : null,
                'likes' => 0,
                'is_liked' => false,
                'created_at' => date('Y-m-d H:i:s'),
            ]
